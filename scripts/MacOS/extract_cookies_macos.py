...
def extract_brave():
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            print(f"DEBUG: rows returned from Brave DB = {len(df)}")
            print(df.head(2) if len(df) > 0 else "DataFrame is empty")
            conn.close()

        # Decrypt
        df["value"] = df["encrypted_value"].apply(lambda x: decrypt_chromium_value(x, aes_key))
        df = df.drop(columns=["encrypted_value"])
...
def extract_firefox():
    ...
    profile_dirs = list(profiles_path.glob("*.default*")) + list(profiles_path.glob("*.default-release*"))
...
def get_chromium_key():
    ...
    for account, service in [
        ...
        try:
            ...
            if password:
                print(f"✅ Retrieved {service} key ({len(password)} chars)")
                return password
...
def extract_chrome():
    ...
    try:
        ...
        cookies = []
        for row in rows:
            domain, name, enc_value, path, expires_utc, secure, httponly, created = row
